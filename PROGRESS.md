...
- 2026-01-16 (控制台展示兼容优化)
- 2026-01-16 (控制台暗色主题)
- 2026-01-16 (控制台卡片紧凑布局)
- 2026-01-16 (服务状态栏布局调整)
- 2026-01-16 (顶部拉取时间与状态简化)
- 2026-01-16 (处理概览统计调整)
- 2026-01-16 (清理示例环境变量中的敏感值)
...
- ✅ **样式修复**：处理长文本换行与表格布局，避免 URL/长内容导致前端溢出
- ✅ **暗色模式**：控制台整体样式切换为暗色主题
- ✅ **布局优化**：服务状态/处理概览改为紧凑卡片，降低高度与间距
- ✅ **状态栏布局**：服务状态标题靠左，Bot/数据库状态靠右并列展示
- ✅ **顶部信息优化**：移除数据来源指示器，新增数据拉取时间并去掉心跳时间显示
- ✅ **概览统计**：处理概览改为两行展示总记录与封禁/放行/人工介入
- ✅ **示例配置清理**：`.env.example` 中的真实 token 改为占位符
...
- NEED_GM 阶段改为固定提及 GM 用户 ID，避免提及机器人角色
...
- `.env.example` 已移除真实密钥，避免推送触发安全拦截
